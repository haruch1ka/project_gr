...
export async function reorganizeIntoFolders(
  ...
  const prompt = `分野：${field}
以下の知識を、内容だけを見て2階層のフォルダ構造に整理してください。
...
- 【重要】既存フォルダ名と同じ意味・近い意味の場合は、必ず既存のフォルダ名をそのまま使うこと。新しい表現を作らない（例：既存に「夜」があれば「夜間」「夜釣り」は使わず「夜」を使う）
- 必ず入力の全itemIdに対して配置先を返すこと

現在のフォルダ（優先的に再利用すること）：${existingList}
